...
static bool IsWindowReallyVisible(HWND hwnd) {
  ...
  // On Windows 11, the "Windows Input Experience" ApplicationFrameWindow
  // reports itself as cloaked even when the touch keyboard is fully visible.
  // We check if it has a visible, uncloaked child CoreWindow.
  wchar_t className[256] = {};
  GetClassName(hwnd, className, 256);
  if (wcscmp(className, L"ApplicationFrameWindow") == 0) {
    g_found_core_window = nullptr;
    EnumChildWindows(hwnd, EnumChildForCoreWindow, 0);
